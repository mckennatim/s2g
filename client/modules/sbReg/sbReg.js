var sbReg = angular.module('sbReg', []);

sbReg.config(['$urlRouterProvider', '$stateProvider', function($urlRouterProvider, $stateProvider){
	$stateProvider.state('register',{
		url:'/register',
		template: '<h4>sbReg</h4><sb-reg></sb-reg>'
	});
	$urlRouterProvider.otherwise('/register');    
}])

sbReg.config(['$httpProvider', function ($httpProvider) {
	$httpProvider.defaults.useXDomain = true;
	delete $httpProvider.defaults.headers.common['X-Requested-With'];
	$httpProvider.interceptors.push('TokenInterceptor');
}])

sbReg.directive('sbReg', ['$state', 'UserService', 'TokenService', 'AuthService', 'cfg', function($state, UserService, TokenService, AuthService,cfg){
	var lsus = cfg.setup().prefix+'users';
	return{
		restrict: 'E',
		scope: {}, 
		templateUrl: "modules/sbReg/sb-reg.html",
		controller: function(){
		},
...
sbReg.factory('AuthService', ['$http', '$q', 'cfg', function($http, $q, cfg) {	
...
sbReg.factory('TokenService', ['cfg', function( cfg){
...
sbReg.factory('TokenInterceptor', ['$q', '$injector', 'cfg', function ($q, $injector, cfg) {
...
sbReg.factory('UserService',  ['$http', 'cfg', '$q', function($http, cfg, $q){
